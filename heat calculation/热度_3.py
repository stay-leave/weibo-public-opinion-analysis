...
def extract_2(inpath):
    ...
    for i in range(1, nrows):#第0行为表头
        alldata = table.row_values(i)#循环输出excel表中每一行，即所有数据
        result_0 = alldata[0]#取出主题
        result_1 = int(alldata[1])#取出转发
        result_2 = int(alldata[2])#取出评论
        result_3 = int(alldata[3])#取出点赞
...
if __name__ == "__main__":
    ...
    for i in lis:
        list_1=extract_1(r'F:\A数据比赛\正式流程及文件\3.数据分析\主题分析\主题新闻数\\'+i+'.xlsx')
        list_2=extract_2('2\\'+i+'热度.xlsx')
        save_afile(run(list_2,list_1),'3\\'+i+'热度.xls')